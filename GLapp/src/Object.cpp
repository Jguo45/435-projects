...
Object::Object(const char* texturePPM, const char* propPPM, const char* normPPM) {
    // create buffer objects to be used later
    glGenTextures(NUM_TEXTURES, textureIDs);
    glGenBuffers(NUM_BUFFERS, bufferIDs);
    glGenVertexArrays(1, &varrayID);
    ...
    // load property image into named texture
    loadPPM(propPPM, textureIDs[PROP_TEXTURE]);

    // load normal image into named texture
    loadPPM(normPPM, textureIDs[NORM_TEXTURE]);
